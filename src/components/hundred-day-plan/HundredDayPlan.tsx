...
const KPIS = [
  { category: "Revenue", color: "#ffffff", metrics: ["Monthly recurring revenue (MRR)", "Net revenue retention %", "Customer churn rate", "Average contract value (ACV)", "New logo acquisition rate", "Pipeline coverage ratio"] },
  { category: "Profitability", color: "#22c55e", metrics: ["EBITDA (monthly, vs. budget)", "Gross margin %", "EBITDA margin %", "Revenue per employee", "Contribution margin by customer", "Variance to model"] },
    ...
    <div style={{minHeight:"100vh",color:"#e5e7eb",padding:"24px 20px",maxWidth:1200,margin:"0 auto"}}>
      ...
      <div style={{marginBottom:20}}>
        <div style={{display:"flex",alignItems:"baseline",gap:12,marginBottom:6}}>
          <div style={{ fontSize:36,letterSpacing:6,color:"#ffffff",lineHeight:1}}>100-DAY</div>
          <div style={{ fontSize:36,letterSpacing:6,color:"rgba(255,255,255,0.1)",lineHeight:1}}>PLAN</div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{fontSize:10,color:prog.pct>70?"#ffffff":"#555",letterSpacing:1}}>{prog.done}/{prog.total} TASKS COMPLETE</div>
        </div>
      </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"220px 1fr",gap:16}}>
          ...
          <div>
            {PILLARS.map(p => {
              ...
              return (
                <div key={p.id} onClick={()=>setActivePillar(p.id)} style={{padding:"12px 14px",marginBottom:8,borderRadius:4,cursor:"pointer",border:`1px solid ${active?p.color+"55":"rgba(255,255,255,0.1)"}`,background:active?p.color+"0e":"#0f1d3d",borderLeft:`3px solid ${active?p.color:p.color+"33"}`,transition:"all .15s"}}>
                  <div style={{ fontSize:13,color:active?p.color:"rgba(255,255,255,0.3)",letterSpacing:2}}>{p.label}</div>
                  <div style={{fontSize:9,color:active?p.color+"aa":"#333",marginTop:3}}>{p.days}</div>
                  <div style={{fontSize:9,color:"rgba(255,255,255,0.2)",marginTop:6,display:"flex",justifyContent:"space-between"}}>
                    <span style={{color:pr.pct===100?"#22c55e":pr.pct>0?"#888":"#333"}}>{pr.done}/{pr.total} done</span>
                  </div>
            ...
            <div style={{marginTop:16,padding:"14px",background:"transparent",border:"1px solid rgba(255,255,255,0.1)",borderRadius:4}}>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",letterSpacing:2,marginBottom:10}}>100-DAY CLOCK</div>
              {[{label:"STABILIZE",days:30,color:"#ffffff"},{label:"ASSESS",days:30,color:"#5a8a9a"},{label:"ACCELERATE",days:40,color:"#22c55e"}].map(s=>(
                <div key={s.label} style={{marginBottom:8}}>
                  ...
                  <div style={{height:6,background:"rgba(255,255,255,0.08)",borderRadius:1,overflow:"hidden"}}>
          ...
          <div>
            <div className="card" style={{padding:"18px 22px",marginBottom:12,borderLeft:`3px solid ${pillar.color}`}}>
              ...
              <div style={{fontSize:13,color:"rgba(255,255,255,0.8)",marginTop:8,marginBottom:6,fontStyle:"italic"}}>{pillar.headline}</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.4)",lineHeight:1.8,borderTop:"1px solid #17305a",paddingTop:10,marginTop:10}}>{pillar.subhead}</div>
            </div>
            ...
            {pillar.items.map((item, i) => {
              ...
              return (
                <div key={i} className="card" style={{marginBottom:6,borderLeft:`3px solid ${item.critical?pillar.color+"33":"rgba(255,255,255,0.1)"}`,opacity:done?0.55:1,transition:"opacity .2s"}}>
      ...
      {tab==="value levers" && (
        <div>
          <div style={{fontSize:11,color:"rgba(255,255,255,0.4)",lineHeight:1.7,marginBottom:20,maxWidth:640}}>
          ...
          {/* Weight bar */}
          <div style={{marginBottom:20}}>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",letterSpacing:2,marginBottom:8}}>WHERE RETURNS COME FROM &mdash; TYPICAL LOWER MIDDLE MARKET PE</div>
            ...
            <div style={{display:"flex",gap:16,marginTop:6,flexWrap:"wrap"}}>
              {VALUE_LEVERS.map(v=>(
                <div key={v.lever} style={{display:"flex",gap:6,alignItems:"center"}}>
                  ...
                  <span style={{fontSize:9,color:"rgba(255,255,255,0.4)"}}>{v.lever}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <div onClick={()=>setExpandedLever(expandedLever===i?null:i)} style={{padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
                <div style={{display:"flex",alignItems:"center",gap:16}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginTop:2}}>{v.examples.slice(0,2).join(" \u00b7 ")}</div>
                  </div>
                </div>
                <span style={{color:"rgba(255,255,255,0.2)",fontSize:12}}>{expandedLever===i?"\u25B2":"\u25BC"}</span>
              </div>
              {expandedLever===i && (
                <div style={{padding:"0 20px 18px",borderTop:"1px solid #17305a"}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:14}}>
                    <div>
                      ...
                      {v.examples.map((e,j)=>(
                        <div key={j} style={{display:"flex",gap:8,marginBottom:7,fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.5}}>
                          <span style={{color:v.color,flexShrink:0}}>&rarr;</span>{e}
                        </div>
                      ))}
                    </div>
                    <div style={{background:"transparent",border:`1px solid ${v.color}22`,borderRadius:4,padding:"14px 16px"}}>
                      <div style={{fontSize:9,color:v.color,letterSpacing:2,marginBottom:8}}>IC READS THIS LEVER AS</div>
                      <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.8}}>{v.icNote}</div>
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="card" style={{padding:"16px 20px",marginTop:8,borderLeft:"3px solid #8a4a4a33"}}>
            <div style={{fontSize:9,color:"#dc2626",letterSpacing:2,marginBottom:8}}>THE MISTAKE MOST OPERATORS MAKE</div>
              ...
              <div onClick={()=>setExpandedMistake(expandedMistake===i?null:i)} style={{padding:"14px 18px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:6}}>
                    <div style={{ fontSize:13,color:"rgba(255,255,255,0.6)",letterSpacing:1}}>{String(i+1).padStart(2,"0")}</div>
                    <div style={{ fontSize:13,color:"#c87a7a",letterSpacing:1}}>{m.mistake.toUpperCase()}</div>
                  </div>
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.4)",lineHeight:1.6}}>{m.cost}</div>
                </div>
                <span style={{color:"rgba(255,255,255,0.2)",fontSize:12,marginLeft:12,flexShrink:0}}>{expandedMistake===i?"\u25B2":"\u25BC"}</span>
              </div>
              {expandedMistake===i && (
                <div style={{padding:"0 18px 16px",borderTop:"1px solid #17305a"}}>
                  <div style={{marginTop:12,padding:"12px 14px",background:"transparent",border:"1px solid #5a8a6a33",borderRadius:4}}>
                    <div style={{fontSize:9,color:"#22c55e",letterSpacing:2,marginBottom:6}}>THE FIX</div>
                    <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.7}}>{m.fix}</div>
                  </div>
                </div>
              )}
            </div>
      ...
      {tab==="kpis" && (
        <div>
              ...
              <div key={i} className="card" style={{padding:"18px 20px",borderLeft:`3px solid ${k.color}33`}}>
                ...
                {k.metrics.map((m, j) => (
                  <div key={j} style={{display:"flex",gap:10,marginBottom:8,alignItems:"flex-start"}}>
                    ...
                    <div style={{fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.5}}>{m}</div>
                  </div>
                ))}
              </div>
            ))}
          ...
          <div className="card" style={{marginTop:12,padding:"16px 20px",borderLeft:"3px solid rgba(255,255,255,0.2)"}}>
            <div style={{fontSize:9,color:"#ffffff",letterSpacing:2,marginBottom:10}}>BOARD REPORTING CADENCE</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,fontSize:10,color:"rgba(255,255,255,0.45)",lineHeight:1.8}}>
              <div><div style={{color:"#ffffff",marginBottom:4,fontSize:9,letterSpacing:1}}>MONTHLY</div>P&amp;L vs. budget &middot; EBITDA &middot; Revenue by segment &middot; Cash position &middot; Variance explanation &middot; Top 3 priorities for next month</div>
              ...
              <div><div style={{color:"#22c55e",marginBottom:4,fontSize:9,letterSpacing:1}}>ANNUALLY</div>Annual plan and budget &middot; Strategic review &middot; Exit readiness assessment &middot; Management incentive review &middot; Board composition review</div>
            </div>
          </div>

          <div className="card" style={{marginTop:12,padding:"16px 20px",borderLeft:"3px solid #8a4a4a33"}}>
            <div style={{fontSize:9,color:"#dc2626",letterSpacing:2,marginBottom:8}}>THE KPI THAT PREDICTS EVERYTHING ELSE</div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.45)",lineHeight:1.8}}>
