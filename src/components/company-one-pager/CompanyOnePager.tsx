  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .tb.on { color: #ffffff; border-bottom: 2px solid #ffffff; }
        ...
        .completion-fill { height: 100%; background: linear-gradient(90deg, rgba(255,255,255,0.3), rgba(255,255,255,0.15)); border-radius: 2px; transition: width .4s ease; }
        @media print {
      ...
      <div className="no-print" style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 4 }}>
          <div style={{ fontSize: 34, letterSpacing: 5, color: "#ffffff", lineHeight: 1 }}>COMPANY</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 10 }}>
          ...
          <div style={{ fontSize: 10, color: completionPct > 70 ? "#ffffff" : "#555", letterSpacing: 1, whiteSpace: "nowrap" }}>
            {completionPct}% COMPLETE
          </div>
          <button onClick={() => window.print()} style={{
            background: "#ffffff", border: "none", color: "#0f1d3d",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 16 }}>
          ...
          <div>
            {FIELD_GROUPS.map((g, i) => (
              <div key={g.label} onClick={() => setActiveGroup(i)} style={{
                ...
                border: `1px solid ${activeGroup === i ? g.color + "66" : "rgba(255,255,255,0.1)"}`,
          ...
          <div className="card" style={{ padding: "20px 24px" }}>
            ...
            {activeGroup === 1 && (ebitdaMarginLY || revGrowth) && (
              <div style={{ marginTop: 20, padding: "12px 14px", background: "rgba(255,255,255,0.02)", border: "1px solid #5a8a6a33", borderRadius: 4 }}>
                <div style={{ fontSize: 9, color: "#22c55e", letterSpacing: 2, marginBottom: 8 }}>AUTO-CALCULATED</div>
                <div style={{ display: "flex", gap: 24, fontSize: 11, color: "#4ade80" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20 }}>
              <button onClick={() => setActiveGroup(g => Math.max(0, g - 1))} disabled={activeGroup === 0}
                style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.4)", padding: "7px 18px", fontSize: 10, letterSpacing: 2, borderRadius: 3, opacity: activeGroup === 0 ? 0.3 : 1 }}>
                \u2190 BACK
              </button>
              <button onClick={() => { setActiveGroup(g => Math.min(FIELD_GROUPS.length - 1, g + 1)); if (activeGroup === FIELD_GROUPS.length - 2) setTab("preview"); }}
                style={{ background: activeGroup === FIELD_GROUPS.length - 1 ? "#ffffff" : "transparent", border: `1px solid ${activeGroup === FIELD_GROUPS.length - 1 ? "#ffffff" : "rgba(255,255,255,0.1)"}`, color: activeGroup === FIELD_GROUPS.length - 1 ? "#080910" : "#555", padding: "7px 18px", fontSize: 10, letterSpacing: 2, borderRadius: 3 }}>
      ...
      {tab === "preview" && (
        <div>
          <div className="no-print" style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={() => window.print()} style={{ background: "#ffffff", border: "none", color: "#080910", padding: "7px 20px", fontSize: 10, letterSpacing: 2, borderRadius: 3 }}>
              PRINT / EXPORT PDF
            </button>
          </div>
          ...
          <div className="onepager">
            ...
            <div className="op-header">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <div style={{ fontSize: 32, letterSpacing: 4, color: "#ffffff", lineHeight: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 16, marginTop: 10, flexWrap: "wrap" }}>
                    {[
                      ...
                      <div key={i} style={{ fontSize: 9, color: "rgba(255,255,255,0.6)", letterSpacing: 1 }}>
                        {item}
                      </div>
                    ))}
                  </div>
                ...
                <div style={{ textAlign: "right" }}>
                  {data.ask && (
                    <div>
                      <div className="op-label" style={{ color: "rgba(255,255,255,0.6)" }}>EQUITY SOUGHT</div>
                      <div style={{ fontSize: 26, color: "#ffffff", letterSpacing: 2 }}>{fmt(data.ask)}</div>
                    </div>
                  )}
                  {(data.entry_multiple || impliedValuation) && (
                    <div style={{ marginTop: 6 }}>
                      <div className="op-label" style={{ color: "rgba(255,255,255,0.6)" }}>ENTRY MULTIPLE</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
              ...
              <div style={{ borderRight: "1px solid #eee" }}>
                ...
                <div className="op-section">
                  <div className="op-label" style={{ color: "#ffffff", marginBottom: 8 }}>BUSINESS OVERVIEW</div>
                ...
                {highlights.length > 0 && (
                  <div className="op-section">
                    <div className="op-label" style={{ color: "#ffffff", marginBottom: 10 }}>INVESTMENT HIGHLIGHTS</div>
                    {highlights.map((h, i) => (
                      <div key={i} className="op-highlight">
                        <div style={{ width: 16, height: 16, borderRadius: "50%", background: "#ffffff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, color: "#080910", flexShrink: 0, marginTop: 1 }}>
                          {i + 1}
                        </div>
                        <div>{h}</div>
                      </div>
                ...
                {data.exit_thesis && (
                  <div className="op-section">
                    <div className="op-label" style={{ color: "#ffffff", marginBottom: 6 }}>EXIT THESIS</div>
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", lineHeight: 1.7, fontStyle: "italic" }}>
                      "{data.exit_thesis}"
                    </div>
                  </div>
                )}
              ...
              <div>
                {/* Financial snapshot */}
                <div className="op-section">
                  <div className="op-label" style={{ color: "#22c55e", marginBottom: 10 }}>FINANCIAL SNAPSHOT</div>
                  <table style={{ width: "100%", fontSize: 10, borderCollapse: "collapse" }}>
                    <thead>
                      <tr style={{ borderBottom: "1px solid #e8e4dc" }}>
                        {["", "2 Yrs Ago", "LY Actual", "CY Est"].map(h => (
                          <td key={h} style={{ padding: "4px 6px", color: "rgba(255,255,255,0.7)", fontSize: 8, letterSpacing: 1 }}>{h}</td>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      <tr style={{ borderBottom: "1px solid #f0ede8" }}>
                        <td style={{ padding: "5px 6px", color: "rgba(255,255,255,0.7)", fontSize: 8 }}>REVENUE</td>
                        <td style={{ padding: "5px 6px", fontWeight: 500 }}>{fmt(data.rev_2y)}</td>
                        <td style={{ padding: "5px 6px", fontWeight: 500 }}>{fmt(data.rev_ly)}</td>
                        <td style={{ padding: "5px 6px", color: "#22c55e", fontWeight: 600 }}>{fmt(data.rev_cy)}</td>
                      </tr>
                      <tr style={{ borderBottom: "1px solid #f0ede8" }}>
                        <td style={{ padding: "5px 6px", color: "rgba(255,255,255,0.7)", fontSize: 8 }}>EBITDA</td>
                        ...
                        <td style={{ padding: "5px 6px", color: "#22c55e", fontWeight: 600 }}>{fmt(data.ebitda_cy)}</td>
                      </tr>
                      <tr>
                        <td style={{ padding: "5px 6px", color: "rgba(255,255,255,0.7)", fontSize: 8 }}>MARGIN</td>
                        ...
                        <td style={{ padding: "5px 6px", color: "#22c55e" }}>{ebitdaMarginCY ? `${ebitdaMarginCY}%` : "\u2014"}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                ...
                {/* Capital structure */}
                <div className="op-section">
                  <div className="op-label" style={{ color: "#22c55e", marginBottom: 10 }}>CAPITAL STRUCTURE & ASK</div>
                  ...
                  {data.use_of_proceeds && (
                    <div style={{ marginTop: 10, fontSize: 9, color: "rgba(255,255,255,0.45)" }}>
                      <span style={{ color: "rgba(255,255,255,0.7)", letterSpacing: 1 }}>USE OF PROCEEDS: </span>{data.use_of_proceeds}
                    </div>
                  )}
                </div>
                ...
                {/* Management */}
                <div className="op-section">
                  <div className="op-label" style={{ color: "#22c55e", marginBottom: 10 }}>MANAGEMENT</div>
                  {[
                    ...
                    <div key={i} style={{ marginBottom: 8, paddingBottom: 8, borderBottom: i < 2 ? "1px solid #f0ede8" : "none" }}>
                      ...
                      {bg && <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>{bg}</div>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ background: "#0f1018", padding: "12px 28px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {(data.contact_name || data.contact_email || data.website) && (
                <div style={{ textAlign: "right", fontSize: 9, color: "rgba(255,255,255,0.6)" }}>
      ...
      {tab === "rules" && (
        <div>
          ...
          {RULES.map((r, i) => (
            <div key={i} className="card" style={{ marginBottom: 10, padding: "16px 20px", borderLeft: "3px solid rgba(255,255,255,0.2)" }}>
              <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                <div style={{ fontSize: 22, color: "rgba(255,255,255,0.27)", flexShrink: 0, lineHeight: 1, marginTop: 2 }}>{i + 1}</div>
                <div>
                  <div style={{ fontSize: 14, color: "#ffffff", letterSpacing: 2, marginBottom: 6 }}>{r.rule.toUpperCase()}</div>
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", lineHeight: 1.7 }}>{r.detail}</div>
                </div>
              </div>
            </div>
          ))}
          ...
          <div style={{ marginTop: 24, padding: "20px 24px", background: "#0a0c0a", border: "1px solid #5a8a6a44", borderRadius: 6, textAlign: "center" }}>
            <div style={{ fontSize: 18, color: "#22c55e", letterSpacing: 4, marginBottom: 8 }}>WAVE 2 COMPLETE</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", lineHeight: 1.9 }}>
